...
class CombinedProtocol(Protocol):
    ...
    def get_stim_sequence(self):
        ...
        s_a = MovingGratingStimulus2(df_param=df, grating_angle=np.pi, grating_period=31.68, wave_shape="sine", clip_mask=[0, 0, 0.5, 1])

        df = pd.DataFrame(dict(t=t, vel_x=vel))
        s_b = MovingGratingStimulus2(df_param=df, grating_angle=0, grating_period=31.68, wave_shape="sine", clip_mask=[0.5, 0, 0.5, 1])
