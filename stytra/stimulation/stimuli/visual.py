...
class CurvedScreenMultipleCirclesStimulus(VisualStimulus, DynamicStimulus):
    """ A filled circle stimulus, which in combination with interpolation
    can be used to make looming stimuli, multiple circles with different luminance
    ...
    def __init__(
        ...
        *args,
        ...
        faster_circles_end_interval=5,
        slower_circles_slowing_rate=0.5,
        slower_circles_n=200,
        **kwargs
    ):
        super().__init__(*args, dynamic_parameters=["x", "y", "radius"], **kwargs)
        self.x = origin[0]
        ...
        self.faster_circles_end_interval = faster_circles_end_interval
        self.slower_circles_slowing_rate = slower_circles_slowing_rate
        self.slower_circles_n = slower_circles_n
    ...
    def paint(self, p, w, h):
        ...
        if self.curved_screen:
            self.radius = self.radius / 180 * (w * mm_px)
            max_loom_radius = self.max_loom_radius / 180 * (w * mm_px)
